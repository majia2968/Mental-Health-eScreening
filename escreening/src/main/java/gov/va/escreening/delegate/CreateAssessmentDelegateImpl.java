...
public class CreateAssessmentDelegateImpl implements CreateAssessmentDelegate {
    ...
    @Resource(name = "programBatteryRepository")
    private ProgramBatteryRepository pbr;
    ...
    @Autowired
    public void setBatterySurveyService(
    ...
    @Autowired
    public void setClinicalReminderSurveyService(
            ClinicalReminderSurveyService clinicalReminderSurveyService) {
    ...
    @Autowired
    public void setVeteranAssessmentNoteService(
            VeteranAssessmentNoteService veteranAssessmentNoteService) {
    ...
    /**
     * The AppProxyName to use for the RPC calls to VistA. This should be
...
//        }
        ...
        for (int i = 0; i < dbResultList.size(); ++i) {
            ...
            selectVeteranResultDto.setVeteranId(dbResultList.get(i)
                    .getVeteranId());
            selectVeteranResultDto.setVeteranIen(dbResultList.get(i)
                    .getVeteranIen());
            selectVeteranResultDto.setSsnLastFour(dbResultList.get(i)
                    .getSsnLastFour());
            selectVeteranResultDto.setFirstName(dbResultList.get(i)
                    .getFirstName());
            selectVeteranResultDto.setMiddleName(dbResultList.get(i)
                    .getMiddleName());
            selectVeteranResultDto.setLastName(dbResultList.get(i)
                    .getLastName());
            selectVeteranResultDto.setBirthDate(dbResultList.get(i)
                    .getBirthDate());
            selectVeteranResultDto.setIsSensitive(dbResultList.get(i)
                    .getIsSensitive());
    ...
    public List<SelectVeteranResultDto> searchVistaForVeterans(
        ...
        for (int i = 0; i < vistaResultList.size(); ++i) {
            ...
            selectVeteranResultDto.setSsnLastFour(vistaResultList.get(i)
                    .getSsnLastFour());
            selectVeteranResultDto.setFirstName(vistaResultList.get(i)
                    .getFirstName());
            selectVeteranResultDto.setMiddleName(vistaResultList.get(i)
                    .getMiddleName());
            selectVeteranResultDto.setLastName(vistaResultList.get(i)
                    .getLastName());
            selectVeteranResultDto.setBirthDate(vistaResultList.get(i)
                    .getBirthDate());
            selectVeteranResultDto.setIsSensitive(vistaResultList.get(i)
                    .getIsSensitive());
            searchResultList.add(selectVeteranResultDto);
        }
    ...
    public VeteranDto fetchVeteran(EscreenUser escreenUser, Integer veteranId,
        ...
        if (veteranId == null && veteranIen == null) {
            ...
            if (forceRefresh == false) {
                if (veteranDto.getDateRefreshedFromVista() == null) {
                    ...
                    long age = System.currentTimeMillis()
                            - veteranDto.getDateRefreshedFromVista().getTime();
            ...
            if (applyMap && escreenUser.getCprsVerified() && veteranIen != null
                ...
                VeteranDto vistaVeteranDto = vistaService.getVeteran(
                        ...
                        escreenUser.getVistaVpid(), escreenUser.getVistaDuz(),
        ...
        } else if (escreenUser.getCprsVerified()
                && StringUtils.isNotEmpty(veteranIen)) {
            // No record exists in DB. So just go get it from VistA.
            veteranDto = vistaService.getVeteran(
                    escreenUser.getVistaDivision(), escreenUser.getVistaVpid(),
    ...
    public List<VistaVeteranAppointment> getVeteranAppointments(
        ...
        List<VistaVeteranAppointment> veteranAppointmentList = vistaService
                .getVeteranAppointments(escreenUser.getVistaDivision(),
                        escreenUser.getVistaVpid(), escreenUser.getVistaDuz(),
                        appProxyName, veteranIen);
    ...
    @Override
    public List<VistaVeteranAppointment> getVeteranAppointments(User user,
                                                                String veteranIen) {
        return vistaService.getVeteranAppointments(user.getVistaDivision(),
                user.getVistaVpid(), user.getVistaDuz(), appProxyName,
                veteranIen);
    }
    ...
    public List<VistaVeteranClinicalReminder> getVeteranClinicalReminders(
        ...
        List<VistaVeteranClinicalReminder> veteranClinicalReminderList = vistaService
                .getVeteranClinicalReminders(escreenUser.getVistaDivision(),
                        escreenUser.getVistaVpid(), escreenUser.getVistaDuz(),
                        appProxyName, veteranIen);
    ...
    @Override
    public List<VeteranAssessmentDto> getVeteranAssessments(Integer veteranId,
                                                            List<Integer> programIdList) {

        List<VeteranAssessmentDto> veteranAssessmentList = new ArrayList<VeteranAssessmentDto>();

        veteranAssessmentList = veteranAssessmentService
                .getVeteranAssessmentsForVeteranAndProgramIdList(veteranId,
                        programIdList);
    ...
    @Override
    public Integer createVeteranAssessment(EscreenUser escreenUser,
                                           int veteranId) {

        return veteranAssessmentService.create(veteranId,
                escreenUser.getUserId());
    }

    @Override
    public List<ClinicalNoteDto> findAllClinicalNoteForProgramId(
            Integer programId) {
    ...
    @Override
    public List<VeteranAssessmentNoteDto> findAllClinicalNoteForVeteranAssessmentId(
            int veteranAssessmentId) {

        return veteranAssessmentNoteService
                .findAllByVeteranAssessmentId(veteranAssessmentId);
    }

    public VeteranAssessment getVeteranAssessmentByVeteranAssessmentId(
            int veteranAssessmentId) {
        VeteranAssessment veteranAssessment = veteranAssessmentService
                .findByVeteranAssessmentId(veteranAssessmentId);
    ...
    public List<SurveyDto> getVeteranAssessmentSurveyList(
        ...
        return surveyService
                .getSurveyListForVeteranAssessment(veteranAssessmentId);
    }
        ...
        if (veteranClinicalReminderDtoList != null) {
            for (VistaVeteranClinicalReminder clinicalReminder : veteranClinicalReminderDtoList) {
                if (clinicalReminder.isDueNow()) {
                    ...
                    // For each survey mapped to this clinical reminder, add a
                    // note entry saying why this survey is
                    // preselected. If it already preselected, append a note for
                    // this clinical reminder.
                    for (int j = 0; j < clinicalReminderSurveyList.size(); ++j) {
                        Integer surveyId = clinicalReminderSurveyList.get(j)
    ...
    public Map<Integer, String> getPreSelectedSurveyMap(
        ...
        List<VistaVeteranClinicalReminder> clinicalReminderList = getVeteranClinicalReminders(
                escreenUser, veteranIen);

        // Get all surveys that should be pre selected and the note to as why it
        // is pre selected.
    ...
    @Override
    public void editVeteranAssessment(EscreenUser escreenUser,
        ...
        veteranAssessmentService.update(veteranAssessmentId, selectedProgramId,
                selectedClinicId, selectedClinicianId, escreenUser.getUserId(),
                selectedNoteTitleId, selectedBatteryId, selectedSurveyIdList);
    ...
    @Override
    public List<DropDownObject> getBatteryListByProgram(Integer pid) {
        List<ProgramBattery> pbList = pbr.findAllByProgramId(pid);
        List<DropDownObject> ddList = pbList
                .stream()
                .map((pb) -> new DropDownObject("" + pb.getBattery().getBatteryId(), pb.getBattery().getName()))
